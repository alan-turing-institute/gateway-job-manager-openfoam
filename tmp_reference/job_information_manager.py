...
###
from schema import Template  #### SQLAlchemy stuff !!!!
...
SSH_PRIVATE_KEY_PATH = os.environ.get("SSH_PRIVATE_KEY_PATH", SSH_PRIVATE_KEY_PATH)
...
SIM_ROOT = os.environ.get("SIM_ROOT", SIM_ROOT)
...
class job_information_manager:
    ...
    def __init__(self, job, job_repository=None):
        ...
        self.user = job.user
        self.extracted_parameters = self._extract_parameters(self.families)
        ...
        self.job_working_directory_name = "{}-{}".format(
            self.case_dir_label, self.job_id
        )
        self.job_working_directory_path = posixpath.join(
            self.simulation_root, self.job_working_directory_name
        )
    ...
    def patch_all_templates(self):
        ...
        for template in self.template_list:
            ...
            self._apply_patch(template_file, self.extracted_parameters, tmp_file)
            patched_tempate = Template(
                source_uri=tmp_file, destination_path=template.destination_path
            )
        ...
        for file_object in all_files:
            ...
            if file_object.destination_path:
                dest_path = posixpath.join(
                    self.job_working_directory_path, file_object.destination_path
                )
        ...
        if to_trigger:
            ...
            if to_trigger.destination_path:
                script_path = posixpath.join(
                    self.job_working_directory_path, to_trigger.destination_path
                )
            else:  # support {"destination_path": null} in job json
