...
def verify_copy(copied_list, destination_dir, ssh_connection):
    ...
    """
    out, err, exit_code = ssh_connection._run_remote_command('find '+destination_dir)
    found_files = out.split("\n")
    for script in copied_list:
        if not script in found_files:
            print("Did not find %s" % script)
            return False
    return True
    
def copy_scripts_to_backend(source_basedir,destination_basedir):
    """
    ...
    ssh_connection._run_remote_command('rm -fr '+destination_basedir)            
    ...
    copied_ok = verify_copy(copied_filenames, destination_basedir, ssh_connection)
    return copied_ok
